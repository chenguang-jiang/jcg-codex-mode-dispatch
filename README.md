...
|---|---|---|
| Contract (`test_skill_contract.py`) | 13 | Priority order, routing table, seven-field dispatch contract, startup gate, GUARD, bilingual README, repo assets |
| Behavior (`test_dispatch.sh`) | 6 | Fail-closed abort, parallel verify, verify-failure collection, Luna→Sol failover, wave barrier, concurrency cap |
| End-to-end (real Codex CLI 0.146) | 1 | 3×Luna parallel + Sol reduce, all checks pass, `exit 0` |

All 19 tests run in CI on every push via [GitHub Actions](.github/workflows/test.yml). Behavior tests use a fake `codex` stub — **zero network, zero cost**.
...
flowchart LR
  ...
  M --> D[dispatch.sh<br/>fifo pool · wave barrier · fail-closed]
  D --> L1[🌙 Luna ∥]
  D --> L2[🌙 Luna ]
  D --> S1[☀️ Sol ∥]
...
- **Failover**: Luna fails → Sol/high rerun (serial safety net, on by default).
...
└── skills/jcg-codex-mode-dispatch/
    ...
    ├── scripts/dispatch.sh
    └── tests/
        ├── test_skill_contract.py   (13 assertions)
        └── test_dispatch.sh         (6 behavior tests)
```
