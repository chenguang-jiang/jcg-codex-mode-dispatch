...
| | 特性 | 说明 |
|---|---|---|
| 🎯 | **准确性优先路由** | Luna *仅*用于输出可机器验证的活；其余一律默认 Sol。拿不准 → Sol 或双跑。绝不为速度牺牲正确性。 |
| 🚦 | **硬性启动门槛** | 每个 `codex exec` 付 ~35–45s 启动税。强制三问自检：≥3 个独立非平凡子任务才派发，否则主线程直接做。 |
| 🛡️ | **三重安全网** | ① 失败关闭：空/非法模型 → `exit 2`，绝不静默回退。② 失败升档：Luna 失败 → 自动 Sol/high 重跑。③ 双跑：两个模型做同一子任务，归约时比对。 |
| ⚡ | **性能优化** | DAG 依赖绕过波次屏障（无木桶效应）· 投机 failover（Sol shadow 预启动）· 轻任务合并（2–4 个轻活 → 1 次启动税）· `EXTRA_EXEC_FLAGS` 跳过插件加载。 |
| 🧪 | **22 项测试，零费用** | 14 契约 + 8 行为测试，每次 push 自动 CI。行为测试用假 `codex` 桩——无网络、无模型调用、无花费。 |
| 🐚 | **纯 bash 3.2** | 除 `codex exec` 外零依赖。Fifo 令牌池用 `read -n1 -u 9`（不用 `wait -n`）。原生 macOS bash 和 CI Linux 均可跑。 |
| 🔒 | **默认隐藏** | `disable-model-invocation: true`——模型永远不会自动触发此 skill。何时值得付启动税，由你决定。 |
